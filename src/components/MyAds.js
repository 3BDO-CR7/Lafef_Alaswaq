...
class MyAds extends Component {
        ...
        axios.post(`${CONST.url}myFreeBlogs`, {
            ...
            user_id :  this.props.auth.data.id,
                ...
                if(response.data.data.length === 0) {
                    Toast.show({
                        text: I18n.translate('no_results'),
                        duration : 2000,
                        type :"danger",
                        textStyle: {
                            color: "white",
    ...
    pay(item){
        let url = 'https://offer-sa.com/make_payment/' + item.price + '/' + item.title + '/' + item.id+ '/' + 'ad' + '/' +  this.props.auth.data.id
    ...
    payment(item){
        let url = 'https://offer-sa.com/make_payment/' + 20 + '/' + 'إعلان تجاري' + '/' + item.id+ '/' + 'photo' + '/' +  this.props.auth.data.id
        this.props.navigation.navigate('Payment' ,{
            url : url
        });
    }
                                                ...
                                                <View style={[styles.block_section_up , { flexBasis : '47%' }]} key={i}>
                                                    <View>
                                                        <View>
                                                            ...
                                                            <TouchableOpacity onPress={()=>{ this.delete(item.id , i ,'photo')}} style={{position: 'absolute', top: -5 , right: -5}}>
                                                            ...
                                                            <TouchableOpacity onPress={()=>{this.props.navigation.navigate('Edit_ad',{id : item.id});}} style={{position: 'absolute', top: -5 , left: -5}}>
                                                                <Icon style={[styles.icons,{ fontSize:18 , color : 'white' ,borderRadius: 50, backgroundColor:'#059220', width: 35 , height:35,textAlign:'center',lineHeight : 35}]} type="AntDesign" name='edit'  />
                                                            </TouchableOpacity>
                                                         </View>
                                                        <View style={[ styles.overHidden, styles.paddingVertical_5, styles.paddingHorizontal_5  ]}>
                                                            ...
                                                            <View style={{ flexDirection:'row', flexBasis : '50%', overflow : 'hidden', alignItems : 'center'}}>
                                                            ...
                                                            <View style={{ flexDirection:'row', width: '100%', alignItems : 'center', justifyContent: 'space-between' }}>
                                                                ...
                                                                <View style={{ flexDirection:'row', flexBasis : '50%', overflow : 'hidden', justifyContent : 'flex-end', alignItems : 'center'}}>
                                                                    <Text style={[ styles.textRegular, styles.text_gray , styles.textSize_11 ]} numberOfLines={1} ellipsizeMode='tail'>
                                                                        { item.date}
                                                                    </Text>
                                                                </View>
